...
useEffect(() => {
  ...
  const calculateStats = (playerMatches: Match[], pId: string) => {
    ...
    comboUsage.forEach((count, combo) => { if (count > maxCombo) { maxCombo = count; mostUsedCombo = combo }})
    finishUsage.forEach((count, finish) => { if (count > maxFinish) { maxFinish = count; favoriteFinish = finish }})
  ...
  const showMatchLogs = (match: Match) => {
    if (!match.match_logs) { setSelectedMatchLogs([]); return }
    try {
      const logs: MatchLog[] = JSON.parse(match.match_logs)
...
if (loading) {
  return (
    <div className="min-h-screen relative overflow-hidden text-white flex items-center justify-center">
      {/* FOND CYBERPUNK (derrière, non interactif) */}
      <div
...
const hasLogs = selectedMatchLogs.length > 0
 ...
 return (
  <div className="min-h-screen relative overflow-hidden text-white">
    ...
    {/* Astuce: on varie taille/position/duration/delay + petit drift via --tx/--ty */}
            ...
            <div className="flip-face flip-back absolute inset-0">
              <div className="bg-black/60 backdrop-blur-md rounded-xl p-6 border border-black h-full">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => { setSelectedMatchLogs([]); setSelectedMatchId(null); }}
