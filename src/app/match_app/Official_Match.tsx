...
export default function TournamentManagementPage() {
  ...
  // ======================================================
  // 📊 Création des decks officiels pour le match
  // ======================================================
  const createOficialMatchDecks = async (matchId: string) => {
    try {
      // Deck du joueur 1
      if (selectedDeck1) {
        const player1Deck = player1Decks.find(d => d.deck_id === selectedDeck1)
        if (player1Deck) {
          const { error: deck1Error } = await supabase
            .from('oficial_matches_decks')
            .insert({
              match_id: matchId,
              player_id: selectedPlayer1,
              combo_id_1: player1Deck.combo_id_1,
              combo_id_2: player1Deck.combo_id_2,
              combo_id_3: player1Deck.combo_id_3,
              deck_id: crypto.randomUUID(),
              Date_Creation: new Date().toISOString()
            })

          if (deck1Error) throw deck1Error
        }
      }

      // Deck du joueur 2
      if (selectedDeck2) {
        const player2Deck = player2Decks.find(d => d.deck_id === selectedDeck2)
        if (player2Deck) {
          const { error: deck2Error } = await supabase
            .from('oficial_matches_decks')
            .insert({
              match_id: matchId,
              player_id: selectedPlayer2,
              combo_id_1: player2Deck.combo_id_1,
              combo_id_2: player2Deck.combo_id_2,
              combo_id_3: player2Deck.combo_id_3,
              deck_id: crypto.randomUUID(),
              Date_Creation: new Date().toISOString()
            })

          if (deck2Error) throw deck2Error
        }
      }

      return true
    } catch (error) {
      console.error('❌ Erreur création decks officiels:', error)
      return false
    }
  }
  ...
  const handleCreateMatch = async () => {
    ...
    // Ensuite créer les decks officiels pour ce match
    const decksCreated = await createOficialMatchDecks(matchId)
    if (!decksCreated) {
      alert('Erreur lors de la création des decks officiels')
      return
    }
